...
export default class GenericController{
    ...
    public edit = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const myObject = await this.myService.edit(req.body as IGenericInterface, req.user as IUsuarioDTO);
            return res.status(200).json({ status: 200, [this.serviceString]: myObject });
        } catch (e) {
    ...
    public delete = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const myObject = await this.myService.delete((req.body as IGenericInterface)._id);
