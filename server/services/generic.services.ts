...
export default class GenericService{
    ...
    public create = async (myObject:IGenericInterface, user?: IUsuarioDTO):Promise<IGenericInterface>=>{
        try {
           ...
           console.log("Hola desde el Generic Service .")
           if (err) throw err;
    ...
    public edit = async (myObject: IGenericInterface, user?: IUsuarioDTO): Promise<IGenericInterface> => {
        try {
            if(typeof user === "undefined"){
    ...
    public delete = async (objectId: string): Promise<Boolean> => {
        try {
